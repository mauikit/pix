...
Cloud::Cloud(QObject *parent) : BaseList (parent)
{
    ...
    connect(this->fm, &FM::cloudItemReady, [this](const FMH::MODEL &item, const QUrl &path)
    {
        ...
        this->update(FMH::toMap(newItem), this->pending.take(QString(item[FMH::MODEL_KEY::PATH]).replace(FMH::CloudCachePath+"opendesktop", FMH::PATHTYPE_LABEL[FMH::PATHTYPE_KEY::CLOUD_PATH])));
...
void Cloud::setList()
{
    emit this->preListChanged();
    this->list.clear();
    this->fm->getCloudServerContent(FMH::PATHTYPE_LABEL[FMH::PATHTYPE_KEY::CLOUD_PATH]+"/"+this->account, FMH::FILTER_LIST[FMH::FILTER_TYPE::IMAGE], 3);
    emit this->postListChanged();
}
